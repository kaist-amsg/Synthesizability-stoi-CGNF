...
from synstoi.data_PU_learning_comp import collate_pool, split_bagging, bootstrap_aggregating, preload_input
from synstoi.model_PU_learning_comp import Net

parser = argparse.ArgumentParser(description='Synthesizability prediction by PU-learning using CGNF representation of composition')
...
parser.add_argument('--bag', default=10, type=int, metavar='N',
...
def main():
    ...
    split_bagging(args.data, args.bag, args.split),
    ...
    for bagging in range(args.restart-1, args.bag):
        ...
        # Load train/valid/test CIF data 
        dataset_train = preload(preload_input = preload_dict, id_prop_file = os.path.join(args.split, 'id_prop_bag_'+str(bagging+1)+'_train.csv'))
        dataset_valid = preload(preload_input = preload_dict, id_prop_file = os.path.join(args.split, 'id_prop_bag_'+str(bagging+1)+'_valid.csv'))
        dataset_test = preload(preload_input = preload_dict, id_prop_file = os.path.join(args.split, 'id_prop_bag_'+str(bagging+1)+'_test-unlabeled.csv'))
